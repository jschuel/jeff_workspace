...
def separate_events(dataframe): #takes read in dataframe and separates multitrack events using DBSCAN with epsilon 2
    ...
    for i in range(0,len(events)):
        ...
        for j in range(0,len(clust_indices)): #loop adds clusters as individual events to dataframe
            new_hit = pd.DataFrame({"event_number": events[i]+j, "timestamp": dataframe['timestamp'][events[i]], "npoints": len(clust_indices[j]), "column": [dataframe['column'][events[i]][clust_indices[j]]], "row": [dataframe['row'][events[i]][clust_indices[j]]], "BCID": [dataframe['BCID'][events[i]][clust_indices[j]]], "tot": [dataframe['tot'][events[i]][clust_indices[j]]], "sum_tot": np.sum(dataframe['tot'][events[i]][clust_indices[j]])},  index=[events[i]+j])
