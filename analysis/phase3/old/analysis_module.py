    ...
    for module in module_ids:
        ...
        for Nb in Nbs:
            ...
            if len(x[module+'_%s'%(Nb)]) > 0:
                ...
                if Nb == 393:
                    gr[module+'_%s'%(Nb)].SetMarkerColor(6)
                if Nb == 789:
...
def extract_variables_LUMI(df, module_ids, bin_width):
    ...
    lumi = [Bin['ECL_LUM_MON_ACC'].mean() for Bin in bin_dfs]
    dlumi = [Bin['ECL_LUM_MON_ACC'].std()/math.sqrt(len(Bin)) for Bin in bin_dfs]
    ...
    P_HER = [Bin['SKB_HER_P_avg'].mean() for Bin in bin_dfs]
    dP_HER = [Bin['SKB_HER_P_avg'].std()/math.sqrt(len(Bin)) for Bin in bin_dfs]
    sy_HER = [Bin['SKB_HER_XRAY_SIGMAY'].mean() for Bin in bin_dfs]
    dsy_HER = [Bin['SKB_HER_XRAY_SIGMAY'].std()/math.sqrt(len(Bin)) for Bin in bin_dfs]
    ...
    P_LER = [Bin['SKB_LER_P_avg'].mean() for Bin in bin_dfs]
    dP_LER = [Bin['SKB_LER_P_avg'].std()/math.sqrt(len(Bin)) for Bin in bin_dfs]
    sy_LER = [Bin['SKB_LER_XRAY_SIGMAY'].mean() for Bin in bin_dfs]
    dsy_LER = [Bin['SKB_LER_XRAY_SIGMAY'].std()/math.sqrt(len(Bin)) for Bin in bin_dfs]
