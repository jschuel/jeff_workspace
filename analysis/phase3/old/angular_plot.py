...
def make_plot(ring, module_id):
    if ring == "LUMI":
        f = "~/data/phase3/PVM/Dec_8_LUMI_updated.root"
        #df = rp.read_root("~/data/phase3/PVM/Dec_8_LUMI_updated.root")
        df = ur.open(f)[ur.open(f).keys()[0]].pandas.df()
        ...
        f_tpc = "~/data/phase3/phase3_background_root/tpc_tools/Dec_7_%s_phase3.root"%(module_id)
        df_tpc = ur.open(f_tpc)['tracks'].pandas.df()
        ...
        f_skb = "~/data/phase3/PVM/Dec_7_%s_updated.root"%(ring)
        df = ur.open(f_skb)[ur.open(f_skb).keys()[0]].pandas.df()
