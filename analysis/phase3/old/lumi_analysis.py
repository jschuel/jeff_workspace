...
input_file = '~/data/phase3/combined_SKB_TPC_ntuples/%s_%s_LUMI_study_new.root'%(month, day)
#Following are for DEC studies...generalize later
HER_input = '~/data/phase3/combined_SKB_TPC_ntuples/Dec_7_HER_study_new.root'
LER_input = '~/data/phase3/combined_SKB_TPC_ntuples/Dec_7_LER_study_new.root'
...
df_HER_input = ur.open(HER_input)[ur.open(HER_input).keys()[0]].pandas.df()
df_LER_input = ur.open(LER_input)[ur.open(LER_input).keys()[0]].pandas.df()
df_Lumi_input = ur.open(input_file)[ur.open(input_file).keys()[0]].pandas.df()
