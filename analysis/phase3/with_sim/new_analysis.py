...
class analysis:
    def __init__(self, E_cut = {'palila': 8, 'iiwi': 8.8, 'tako': 6, 'nene': 5.5, 'elepaio': 6, 'humu': 6.6}, skb_file= "/home/jeff/data/phase3/spring_2020/05-09-20/combined_ntuples/05-09_whole_study_even_newerest.root", tpc_path =  '/home/jeff/data/phase3/spring_2020/05-09-20/tpc_root_files/', recoils_only = True, fei4_restrict = True, data_machine_params = {'I_HER' : 510, 'I_LER' : 510, 'sy_LER' : 60, 'sy_HER' : 35, 'nb_LER' : 783, 'nb_HER' : 783, 'lumi' : 1.1}, MC_machine_params = {'I_HER' : 1000, 'I_LER' : 1200, 'sy_LER' : 37, 'sy_HER' : 36, 'nb_LER' : 1576, 'nb_HER' : 1576, 'lumi' : 25}, bin_width = 120):
        self.E_cut = E_cut
    ...
    def combine_tpc_and_SKB_data(self):
        ...
        for tpc in self.tpcs.values():
            ...
            cinj_flag = []
            decay_flag = []
            for Il, Ih, Syl, Syh, Nbl, Nbh, lumi, rate, fler, fher, flum, fci, fd in zip(tmp['I_LER'], tmp['I_HER'], tmp['Sy_LER'], tmp['Sy_HER'], tmp['Nb_LER'], tmp['Nb_HER'], tmp['ECL_lumi'], tmp['%s_neutrons'%(tpc)], tmp['LER_study_flag'], tmp['HER_study_flag'], tmp['Lumi_study_flag'],tmp['Cont_inj_flag'], tmp['Decay_flag']):
                if rate > 1:
    ...
    def bin_data(self,nsecs): #nsecs is number of seconds per bin
        ...
        for tpc in self.tpcs.values():
            ...
            for col in ['ts', 'I_LER', 'I_HER', 'Sy_LER', 'Sy_HER', 'Nb_LER', 'Nb_HER', 'Lumi']:
    ...
    def apply_fits_to_data(self, period, cont_inj = True, decay = True):
        ...
        for tpc in self.tpcs.values():
            ...
            if cont_inj and decay:
                reduced_data[tpc] = data[tpc]#.loc[(data[tpc][period+'_flag'] == 1)]
                if period == 'Lumi':
                    reduced_data[tpc]['BG_pred'] = fits[tpc+'_HER_B1']*reduced_data[tpc]['I_HER']**2 + fits[tpc+'_LER_B1']*reduced_data[tpc]['I_LER']**2
                    ...
                    reduced_data[tpc]['BG_pred_err'] = np.sqrt((reduced_data[tpc]['I_HER']**2*fits[tpc+'_HER_B1_err'])**2 + (2*reduced_data[tpc]['I_HER']*reduced_data[tpc]['I_HER_err']*fits[tpc+'_HER_B1'])**2 + (reduced_data[tpc]['I_LER']**2*fits[tpc+'_LER_B1_err'])**2 + (2*reduced_data[tpc]['I_LER']*reduced_data[tpc]['I_LER_err']*fits[tpc+'_LER_B1'])**2)
                    ...
                    reduced_data[tpc]['BG_pred'] = fits[tpc+'_'+period+'_B1']*reduced_data[tpc]['I_'+period]**2
                    ...
                    reduced_data[tpc]['BG_pred_err'] = np.sqrt((reduced_data[tpc]['I_'+period]**2*fits[tpc+'_'+period+'_B1_err'])**2 + (2*reduced_data[tpc]['I_'+period]*reduced_data[tpc]['I_'+period+'_err']*fits[tpc+'_'+period+'_B1'])**2)
                ...
                if period == 'Lumi':
                    reduced_data[tpc]['BG_pred'] = fits[tpc+'_HER_Decay_B1']*reduced_data[tpc]['I_HER']**2 + fits[tpc+'_LER_Decay_B1']*reduced_data[tpc]['I_LER']**2
                    ...
                    reduced_data[tpc]['BG_pred_err'] = np.sqrt((reduced_data[tpc]['I_HER']**2*fits[tpc+'_HER_Decay_B1_err'])**2 + (2*reduced_data[tpc]['I_HER']*reduced_data[tpc]['I_HER_err']*fits[tpc+'_HER_Decay_B1'])**2 + (reduced_data[tpc]['I_LER']**2*fits[tpc+'_LER_Decay_B1_err'])**2 + (2*reduced_data[tpc]['I_LER']*reduced_data[tpc]['I_LER_err']*fits[tpc+'_LER_Decay_B1'])**2)
                    ...
                    reduced_data[tpc]['BG_pred'] = fits[tpc+'_'+period+'_Decay_B1']*reduced_data[tpc]['I_'+period]**2
                    ...
                    reduced_data[tpc]['BG_pred_err'] = np.sqrt((reduced_data[tpc]['I_'+period]**2*fits[tpc+'_'+period+'_Decay_B1_err'])**2 + (2*reduced_data[tpc]['I_'+period]*reduced_data[tpc]['I_'+period+'_err']*fits[tpc+'_'+period+'_Decay_B1'])**2)
                ...
                if period == 'Lumi':
                    reduced_data[tpc]['BG_pred'] = fits[tpc+'_HER_Cont_inj_B1']*reduced_data[tpc]['I_HER']**2 + fits[tpc+'_LER_Cont_inj_B1']*reduced_data[tpc]['I_LER']**2
                    reduced_data[tpc]['T_pred'] = fits[tpc+'_HER_Cont_inj_T']*reduced_data[tpc]['I_HER']**2/(reduced_data[tpc]['Sy_HER']*reduced_data[tpc]['Nb_HER']) + fits[tpc+'_LER_Cont_inj_B1']*reduced_data[tpc]['I_LER']**2/(reduced_data[tpc]['Sy_LER']*reduced_data[tpc]['Nb_LER'])

                    reduced_data[tpc]['BG_pred_err'] = np.sqrt((reduced_data[tpc]['I_HER']**2*fits[tpc+'_HER_Cont_inj_B1_err'])**2 + (2*reduced_data[tpc]['I_HER']*reduced_data[tpc]['I_HER_err']*fits[tpc+'_HER_Cont_inj_B1'])**2 + (reduced_data[tpc]['I_LER']**2*fits[tpc+'_LER_Cont_inj_B1_err'])**2 + (2*reduced_data[tpc]['I_LER']*reduced_data[tpc]['I_LER_err']*fits[tpc+'_LER_Cont_inj_B1'])**2)
                    reduced_data[tpc]['T_pred_err'] = np.sqrt((reduced_data[tpc]['I_HER']**2/(reduced_data[tpc]['Sy_HER']*reduced_data[tpc]['Nb_HER'])*fits[tpc+'_HER_Cont_inj_T_err'])**2 + (2*reduced_data[tpc]['I_HER']*fits[tpc+'_HER_Cont_inj_T']/(reduced_data[tpc]['Sy_HER']*reduced_data[tpc]['Nb_HER'])*reduced_data[tpc]['I_HER_err'])**2 +(reduced_data[tpc]['I_HER']**2*fits[tpc+'_HER_Cont_inj_T']/(reduced_data[tpc]['Sy_HER']**2*reduced_data[tpc]['Nb_HER'])*reduced_data[tpc]['Sy_HER_err'])**2 + (reduced_data[tpc]['I_LER']**2/(reduced_data[tpc]['Sy_LER']*reduced_data[tpc]['Nb_LER'])*fits[tpc+'_LER_Cont_inj_T_err'])**2 + (2*reduced_data[tpc]['I_LER']*fits[tpc+'_LER_Cont_inj_T']/(reduced_data[tpc]['Sy_LER']*reduced_data[tpc]['Nb_LER'])*reduced_data[tpc]['I_LER_err'])**2 +(reduced_data[tpc]['I_LER']**2*fits[tpc+'_LER_Cont_inj_T']/(reduced_data[tpc]['Sy_LER']**2*reduced_data[tpc]['Nb_LER'])*reduced_data[tpc]['Sy_LER_err'])**2)
                else:
                    reduced_data[tpc]['BG_pred'] = fits[tpc+'_'+period+'_Cont_inj_B1']*reduced_data[tpc]['I_'+period]**2
                    reduced_data[tpc]['T_pred'] = fits[tpc+'_'+period+'_Cont_inj_T']*reduced_data[tpc]['I_'+period]**2/(reduced_data[tpc]['Sy_'+period]*reduced_data[tpc]['Nb_'+period])

                    reduced_data[tpc]['BG_pred_err'] = np.sqrt((reduced_data[tpc]['I_'+period]**2*fits[tpc+'_'+period+'_Cont_inj_B1_err'])**2 + (2*reduced_data[tpc]['I_'+period]*reduced_data[tpc]['I_'+period+'_err']*fits[tpc+'_'+period+'_Cont_inj_B1'])**2)
                    reduced_data[tpc]['T_pred_err'] = np.sqrt((reduced_data[tpc]['I_'+period]**2/(reduced_data[tpc]['Sy_'+period]*reduced_data[tpc]['Nb_'+period])*fits[tpc+'_'+period+'_Cont_inj_T_err'])**2 + (2*reduced_data[tpc]['I_'+period]*fits[tpc+'_'+period+'_Cont_inj_T']/(reduced_data[tpc]['Sy_'+period]*reduced_data[tpc]['Nb_'+period])*reduced_data[tpc]['I_'+period+'_err'])**2 +(reduced_data[tpc]['I_'+period]**2*fits[tpc+'_'+period+'_Cont_inj_T']/(reduced_data[tpc]['Sy_'+period]**2*reduced_data[tpc]['Nb_'+period])*reduced_data[tpc]['Sy_'+period+'_err'])**2)
