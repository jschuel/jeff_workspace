...
def get_sim_rates(bg_type, ring, pressure):
    
    f = ROOT.TFile("/home/jeef/data/phase3/beast_fast_simulation/beast_hist/%s_%s_%s.root"%(bg_type,ring,pressure))
    ...
    recoil_e = ROOT.TH3F(f.Get("h_mctpc_recoil_w_He"))
    h_tpc_recoil = ROOT.TH1F("tpc_rate", "Recoil Rates", 6, -0.5, 5.5)
    module_id = ['humu','nene','iiwi','palila','tako','elepaio']
    sim_rates = {}

    i = 0
    for tpc in module_id:
        ht = ROOT.TH1D(recoil_e.ProjectionX("tmp",i+1,i+1))
        sim_rates[tpc] = ht.Integral(0,ht.GetXaxis().GetNbins()-1)/t
        h_tpc_recoil.SetBinContent(i+1, sim_rates[tpc])
        i+=1
